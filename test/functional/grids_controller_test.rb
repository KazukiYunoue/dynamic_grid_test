...
class GridsControllerTest < ActionController::TestCase
  ...
  test "should create grid" do
    assert_difference('Grid.count') do
      post :create, grid: { description: @grid.description, image: @grid.image, name: @grid.name, views_count: @grid.views_count }
    end
  ...
  test "should update grid" do
    put :update, id: @grid, grid: { description: @grid.description, image: @grid.image, name: @grid.name, views_count: @grid.views_count }
    assert_redirected_to grid_path(assigns(:grid))
  end
